...
We would not expect you to spend more than 2-3 hours on this - do what you can in the time allowed. Ensure each task is committed in a separate commit.
...
## Setup
* Copy this project to a new private repo on github, using best practices for your commits.
...
* There is an admin panel that is only accessible to logged in users, there is no link to it on the website. You can navigate to `/admin` to access it.

## Initial Tasks
* The expired_at field is not saving when creating OR editing a blog post in the admin panel. Investigate and fix.
...
* When clicking through to a blog post from `/blog` page, we are getting a 404. Investigate and fix.
...
* Add mchlbwyr as a contributor to your new repository on GitHub.
* Take a backup of your database and email it to mbowyer@tariffcom.com along with your GitHub username.
